...
export function GuestApp({ view, productId, menuData }: { view: View; productId?: string; menuData?: GuestMenuData }) {
  ...
  else if (view === "product") content = <ProductDetail product={data.products.find((item) => item.id === productId)} onAdd={addToCart} />;
  ...
  return <div className="guest-app"><a className="skip-link" href="#main-content">Spring til indhold</a><AppHeader view={view} cartCount={count} /><main id="main-content" tabIndex={-1} className="page-content">{content}</main><BottomNav active={view} /><div className="toast" aria-live="polite">{toast}</div></div>;
}

const headerBackByView: Partial<Record<View, string>> = { product: "/menu", cart: "/menu", checkout: "/kurv", "previous-detail": "/tidligere" };
function AppHeader({ view, cartCount }: { view: View; cartCount: number }) {
  const back = headerBackByView[view];
...
type NavIconName = "home" | "menu" | "history" | "offers";
function NavIcon({ name }: { name: NavIconName }) {
  const shared = { viewBox: "0 0 24 24", fill: "none" as const, stroke: "currentColor", strokeWidth: 1.8, strokeLinecap: "round" as const, strokeLinejoin: "round" as const, "aria-hidden": true };
  if (name === "home") return <svg {...shared}><path d="M4 11.5 12 5l8 6.5" /><path d="M6 10v9h5v-5h2v5h5v-9" /></svg>;
  if (name === "menu") return <svg {...shared}><path d="M8 3v6a2 2 0 0 0 4 0V3" /><path d="M10 9v12" /><path d="M16 3c2.2 2 2.2 5.4 0 7.4L15 11v10" /></svg>;
  if (name === "history") return <svg {...shared}><circle cx="12" cy="12" r="8.5" /><path d="M12 7.5V12l3 2.5" /></svg>;
  return <svg {...shared}><path d="M20 12.5 12.5 20 4 11.5V4h7.5z" /><circle cx="8" cy="8" r="1.1" fill="currentColor" stroke="none" /></svg>;
}
function BottomNav({ active }: { active: View }) {
  const links: Array<{ label: string; href: string; icon: NavIconName; active: boolean }> = [
    { label: "Hjem", href: "/", icon: "home", active: active === "home" },
    { label: "Menu", href: "/menu", icon: "menu", active: ["menu", "product", "cart", "checkout"].includes(active) },
    { label: "Tidligere", href: "/tidligere", icon: "history", active: ["previous", "previous-detail"].includes(active) },
    { label: "Tilbud", href: "/tilbud", icon: "offers", active: active === "offers" },
  ];
  return <nav className="bottom-nav" aria-label="Hovednavigation">{links.map((link) => <Link key={link.label} className={link.active ? "active" : ""} aria-current={link.active ? "page" : undefined} href={link.href}><NavIcon name={link.icon} />{link.label}</Link>)}</nav>;
}
