...
export async function getActiveMenuFromNeon(now = new Date()): Promise<MenuReadModel> {
  const db = getDb();
  const [categoryRows, productRows, optionRows, allergenRows, offerRows] = await Promise.all([
  ...
  return {
    ...
    offers: offerRows.map((offer) => ({ id: offer.id.replace("offer-", ""), title: offer.title, badge: offer.badge, description: offer.description, imagePath: offer.imagePath })),
    source: "neon",
  };
}
...
export async function getMenuReadModel(): Promise<MenuReadModel> {
  if (!process.env.DATABASE_URL) {
    return { categories: fixtureCategories, products: fixtureProducts, offers: fixtureOffers, source: "fixtures" };
  }
